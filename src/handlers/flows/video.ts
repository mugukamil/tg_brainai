...
export async function handleVideoGeneration(
  ...
  if (!text) return;
  if (!isVideoAvailable()) {
    await bot.sendMessage(chatId, '❌ Генерация видео в настоящее время недоступна. Обратитесь в службу поддержки.');
    return;
  }
  ...
  if (!canUse) {
    await bot.sendMessage(chatId, '❌ У вас закончились видео-запросы. Оформите премиум в разделе ✨ Премиум.');
    return;
  }
  ...
  try {
    ...
    if (!start.success || !start.data?.prediction_id) {
      await safeEditMessageText(bot, chatId, statusMsg.message_id, `❌ Ошибка запуска: ${start.error?.message || 'unknown'}`);
      return;
    }
      ...
      try {
        ...
        if (!st.success) {
          ...
          if (transientErrors >= maxTransientErrors) {
            clearInterval(timer);
            await safeEditMessageText(bot, chatId, statusMsg.message_id, `❌ Ошибка статуса: ${st.error?.message || 'Неизвестная ошибка'}`);
            return;
          }
        ...
        if (status === 'succeeded') {
          ...
          if (url) {
            await (bot as any).sendVideo(chatId, url, { caption: `Видео по запросу: ${params.prompt}` });
            await decreaseRequests(userId, 'video_req_left' as any, 1);
            try { await bot.deleteMessage(chatId, statusMsg.message_id); } catch {}
          } else {
            await safeEditMessageText(bot, chatId, statusMsg.message_id, '✅ Видео готово, но URL не получен.');
          }
        } else if (status === 'failed') {
          clearInterval(timer);
          await safeEditMessageText(bot, chatId, statusMsg.message_id, '❌ Задача не выполнена. Попробуйте другой запрос.');
        } else if (attempts >= maxAttempts) {
          clearInterval(timer);
          await safeEditMessageText(bot, chatId, statusMsg.message_id, '⏰ Тайм-аут ожидания видео. Попробуйте снова.');
        }
      } catch (e) {
        clearInterval(timer);
        await safeEditMessageText(bot, chatId, statusMsg.message_id, '❌ Ошибка мониторинга видео.');
      }
    }, 5000);
  } catch (e: any) {
    await safeEditMessageText(bot, chatId, statusMsg.message_id, `❌ Ошибка: ${e?.message || 'unknown'}`);
  }
}
