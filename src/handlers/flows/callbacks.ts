...
import { findUser, hasRequestsLeft, decreaseRequests } from '../supabase-handler.js';
...
export async function handleCallbackQuery(
  ...
  if (index === undefined) {
    await bot.answerCallbackQuery(callbackQuery.id, { text: '❌ Неверный индекс', show_alert: true });
    return;
  }
  let user = await findUser(userId);
  if (!user || !hasRequestsLeft(user, 'image_req_left')) {
    await bot.answerCallbackQuery(callbackQuery.id, { text: '❌ Нет запросов на улучшение изображения', show_alert: true });
    return;
  }
  ...
  try {
    ...
    if (!upscaleResult.success) {
      await safeEditMessageText(bot, chatId, statusMsg.message_id, `❌ Улучшение изображения не удалось: ${upscaleResult.error?.message}`);
      return;
    }
    const newTaskId = upscaleResult.data?.data?.task_id;
    if (!newTaskId) {
      await safeEditMessageText(bot, chatId, statusMsg.message_id, '❌ Нет task ID улучшения изображения');
      return;
    }
        ...
        if (statusResult.success) {
          ...
          if (status === 'completed') {
            ...
            if (imageUrl) {
              ...
              await safeEditMessageText(bot, chatId, statusMsg.message_id, '❌ Улучшение изображения завершено, но нет изображения');
            }
            ...
            clearInterval(pollInterval);
            await safeEditMessageText(bot, chatId, statusMsg.message_id, '⏰ Улучшение изображения завершено');
          }
        }
      } catch (error) {
        clearInterval(pollInterval);
