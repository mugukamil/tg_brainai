...
function getAnchoredPeriodForUser(
    ...
    const now = new Date();
    if (user.is_premium) {
        const source = (user as any).premium_started_at || (user as any).created_at || now;
        const anchor = new Date(source as any);
    ...
    const freeSource = (user as any).free_period_start || (user as any).created_at || now;
    const anchor = new Date(freeSource as any);
    let periodStart = new Date(
...
async function getOrCreateCurrentQuotaRow(userDbId: string): Promise<QuotaRow | null> {
    ...
    const { start, end } = userRow
        ? getAnchoredPeriodForUser(userRow as any)
        : getCurrentWeekPeriodUTC();
 ...
 */
export async function getAllUsers(limit: number = 100, offset: number = 0): Promise<DbUser[]> {
    try {
        const { data, error } = await supabase
            .from("gpt_tg_users")
            .select("*")
